...
- The application is an academic prototype rather than a production e-commerce platform.
- The current Windows Forms dashboard focuses on read-only browsing and analytics rather than complete CRUD workflows.
- Tax is fixed at 5% in the course implementation.
- Shipping prices are static and keyed by country.
- The seed script is resource-intensive because it creates one million orders.
- The database connection is configured through application configuration files and must be secured before public deployment.
- No authentication, role-based access control, payment-gateway integration, or automated test project is included.
- Generated build folders and locally restored packages should not be committed to the repository.

---

## Future Improvements

- Add complete create, update, and cancellation workflows to the desktop interface.
- Add authentication and role-based access for administrators, staff, and analysts.
- Replace fixed tax and shipping rules with configurable policy tables.
- Add pagination and asynchronous loading for million-row datasets.
- Add transaction logging and an audit trail for inventory changes.
- Add automated unit, integration, and database tests.
- Introduce Entity Framework migrations or versioned database deployment scripts.
- Store secrets outside source-controlled configuration files.
- Add charts and export options for reports.
- Develop a web-based or REST API version of the application.

---

## Contributors

| Contributor | Roll Number |
|---|---:|
| Wajdan Rasool | 27100319 |
| Yahya Ali | 27100160 |
| Shahab Ud Din | 27100124 |
| Syed Muhammad Abdullah | 27100336 |

---

## License

This academic project currently does not include an open-source license. Unless a license is added, the source code remains subject to the default copyright restrictions of its authors.
